...
public class ReentrantReadWriteLock {
    ...
    public synchronized void lockRead() {
        ...
        //log.debug("" + Thread.currentThread().getId() + " locked read, readlock.size() = " + reader.size());
    }
    ...
    public synchronized void unlockRead() {
        ...
        if (isReadLockHeldByCurrentThread()) {
            ...
            //log.debug("" + currentThreadId + " unlocked read, readlock.size() = " + reader.size());
        }
    ...
    public synchronized void lockWrite() {
        while (!tryAcquiringWriteLock()) {
            try {
                ...
                log.error("Exception while running the lockWrite: " + e);
            }
        }

        //log.debug("" + Thread.currentThread().getId() + " locked write, writelock.size() = " + writer.size());
    }
    ...
    public synchronized void unlockWrite() {
        ...
        if (isWriteLockHeldByCurrentThread()) {
            ...
            //log.debug("" + currentThreadId + " unlocked write, writelock.size() = " + writer.size());
        }
